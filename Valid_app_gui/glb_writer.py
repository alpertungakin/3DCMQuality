    ...
    def create_glb(city_objects: List[str], cm: cityjson.CityJSON, transformer) -> Tuple[GLTF2, List[float], List[float]]:
        ...
        for obj_id in city_objects:
            ...
            for surface in geometry.boundaries:
                ...
                triangles = ec115.earcut(flat_coords, None, 3)
        ...
        # Recenter vertices on the model centroid before casting to float32, so
        # large CRS offsets (UTM hundreds-of-thousands, ECEF millions) don't
        # destroy float32 precision in the GLB. Centroid math is done in
        # float64; the resulting deltas fit comfortably in float32.
        verts3 = np.array(vertices, dtype=np.float64).reshape(-1, 3)
        if verts3.size == 0:
            raise ValueError("glb_writer.create_glb produced zero vertices — check that the CityJSON geometry has dereferenced boundaries (use cityjson.load, not cityjson.reader).")
        centroid = verts3.mean(axis=0)
        verts3 -= centroid
        vertices = verts3.astype(np.float32).flatten()
        ...
        return gltf, min_vals, max_vals, centroid.tolist()