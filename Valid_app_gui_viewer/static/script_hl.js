...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function viewWhole(glbdata) {
        // Reference the #fullView container
        ...
        const camera = new THREE.PerspectiveCamera(
            75, 
            container.offsetWidth / container.offsetHeight, // Set aspect ratio
            0.1, 
            1000
        );
        ...
        container.appendChild(renderer.domElement); // Append renderer to #fullView
        ...
        loader.load(
            ...
            function (gltf) {
                ...
                if (selectedID){
                    selectedOBJ = findObjectByNameOrUUID(model, selectedID);
                }
        ...
        const controls = new THREE.OrbitControls(camera, renderer.domElement);
        controls.enableDamping = true; // Enable inertia for smoother movement
        ...
        controls.minDistance = 1; // Optional: set minimum distance from the camera to the object
        controls.maxDistance = 1000; // Optional: set maximum distance from the camera to the object
    
        let isAutoZooming = true; // Flag to control automatic zoom behavior
        let originalMaterial = null; // Store the original material of the selected object
        ...
        function animate() {
            ...
            // Only perform automatic zoom if isAutoZooming is true
            if (selectedOBJ && isAutoZooming) {
                ...
                // Make the object transparent during auto-zoom
                if (!originalMaterial) {
                    originalMaterial = selectedOBJ.material.clone(); // Clone the original material
                    selectedOBJ.material = originalMaterial; // Assign the cloned material to the object
                }
                selectedOBJ.material.transparent = true; // Enable transparency
                selectedOBJ.material.opacity = 0.01; // Set the object opacity to 50% during zoom
                ...
                camera.position.lerp(newPosition, 0.05); // Smoothing factor
        
                // Ensure the camera looks at the object
                camera.lookAt(controls.target);
            } else {
                // Restore the object's material when auto-zooming is disabled
                if (selectedOBJ && originalMaterial) {
                    selectedOBJ.material = originalMaterial; // Restore the original material
                    selectedOBJ.material.transparent = false; // Disable transparency
                    selectedOBJ.material.opacity = 1; // Reset opacity to fully visible
